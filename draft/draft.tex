...
mechanism\href{http://bitfury.com/content/5-white-papers-research/pos-vs-pow-1.0.2.pdf}{PoW
vs PoS} for the platform.

\subsection{1.3 Personal Aims}\label{personal-aims}

Having been a follower of the
bitcoin\href{https://bitcoin.org/en/}{(Bitcoin)} movement for more than
...
Native\href{https://facebook.github.io/react-native/}{(RN)}. - Learn how
to write smart contracts in Solidity \emph{\href{}{REF}} that are useful
in the context of the project and for existing platforms such as
Ethereum \emph{\href{}{REF}}. - Implement a form of blockchain
technology, by learning how to set up, run \& maintain a a blockchain
proficiently. - Gain a fundamental understanding of Docker
\emph{\href{}{REF}} and how it leverages ``containerisation'' to achieve
...
development\href{http://tocs.ulb.tu-darmstadt.de/82047766.pdf}{(Jacobson,
Ivar, et al. The unified software development process. Vol. 1. Reading:
Addison-wesley, 1999.)}, and was thus structured in terms of iterative
...
categorisation scheme \emph{\href{}{REF}}.

During the implementation stage of the project, a daily log (\textbf{see
Appendix X}) was kept of the key advances and issues encountered during
...
Issues\href{}{(REF)} and organised into a Kanban board\href{}{(REF)}
with the help of waffle.io\href{https://waffle.io/}{(waffle.io)}.
...
Additionally, a Gantt chart (\textbf{see Appendix X}) was established at
the outset of the project in order to visualise the progression of the
project over time and to ensure the amount of work set out to be
completed was within a realistic margin for the amount of time available
to the author.
...
little effort on school work
\href{http://cep-dc.org/displayDocument.cfm?DocumentID=405}{(Center on
Education Policy 2012)}. More worryingly still, 70\% of dropouts stated
lack of motivation, in a 2006 study, as the key reason for their
departure from school \href{}{(Bridgeland, J.M. et al., 2006. The Silent
Epidemic: Perspectives of High School Dropouts)}. In liaison with these
...
not belonging as the reason for their departure from school
\href{}{(Berktold et al., 1998, Table 6)}.
...
motivating them to remain engaged in the process \href{}{((Johnson et
al., 2001; Newmann, Wehlage, and Lamborn, 1992; Tinto, 1993; Wehlage,
Rutter, Smith, Lesko, and Fernandez, 1989))}. Of course, simply being
...
constitute attention to what is being taught \href{}{(\emph{ibid})}.
...
control, such as whether they earn an A grade.''\\
\href{}{(Usher, Kober; Student Motivation - An overlooked piece of
school reform)}
\end{quote}

\textbf{Figure X} visualises a widely held theory regarding educational
conditions that promote intellectual engagement\href{}{(Committee on
Increasing High School Students' Engagement and Motivation to Learn \&
National Research Council 2003)}. The proposed system would thus provide
...
\paragraph{\texorpdfstring{SocialX\protect\hyperlink{needsLink}{(Learning
from peers: motivating students through reputation systems )},
\protect\hyperlink{needsLink}{(Collaborative projects and self
evaluation within a social reputation-based exercise-sharing system
)}}{SocialX(Learning from peers: motivating students through reputation systems ), (Collaborative projects and self evaluation within a social reputation-based exercise-sharing system )}}\label{socialxlearning-from-peers-motivating-students-through-reputation-systems-collaborative-projects-and-self-evaluation-within-a-social-reputation-based-exercise-sharing-system}

SocialX is an exercise sharing tool which enables students to earn
...
\paragraph{\texorpdfstring{WikiSpaces
Classroom\href{https://www.wikispaces.com/content/classroom/about}{(WikiSpaces)}}{WikiSpaces Classroom(WikiSpaces)}}\label{wikispaces-classroomwikispaces}

WikiSpaces is a web app which provides a virtual classroom workspace in
...
\paragraph{\texorpdfstring{HaikuLearning\href{https://www.haikulearning.com/}{(HaikuLearning)}}{HaikuLearning(HaikuLearning)}}\label{haikulearninghaikulearning}

HaikuLearning, similarly to WikiSpaces, is a web app which attempts to
...
the chain\href{}{(Mastering Bitcoin)}. Each block is identified by a
hash, which is stored in the block's header alongside the previous
block's hash, known as the parent block(\emph{ibid}), thus providing a
...
more
parties\href{http://szabo.best.vwh.net/smart_contracts_idea.html}{(smart\_contracts\_idea)}.
...
OpenChain\href{https://www.openchain.org/}{(OpenChain)}, which seemed to
...
MultiChain\href{http://www.multichain.com/}{(MultiChain)}, on the other
...
Eris\href{https://erisindustries.com/}{(Eris)} platform. Eris itself
provides a suite of tools which wrap and augment a
Tendermint\href{http://tendermint.com/}{(Tendermint)} blockchain
...
Solidity\href{http://solidity.readthedocs.io/en/latest/}{(Solidity)}
...
NodeJS\href{https://nodejs.org/en/}{(NodeJS)}. NodeJS provides a great
...
NPM\href{https://www.npmjs.com/}{(npm)}, thus providing an essential
...
ECMAScript\href{http://www.ecma-international.org/}{(ECMAScript)} 5
...
Express\href{https://expressjs.com/}{(Express)} is a minimalist
...
structured\href{https://github.com/eris-ltd/hello-eris}{(Hello Eris)}.\\
...
Async\href{https://caolan.github.io/async/}{(Async)} is a utility
...
app with the Apache Cordova\href{https://cordova.apache.org/}{(Cordova)}
...
React\href{https://facebook.github.io/react/}{(React)} web development
library, in liaison with Dan Abramov's
Redux\href{http://redux.js.org/}{(Redux)} library for state management,
...
React Native\href{https://facebook.github.io/react-native/}{(React
Native)} for the project, which grafts a truly native mobile development
...
is hard to achieve in a traditional Model-View-Controller\href{}{(REF)}
...
redux-thunk\href{https://github.com/gaearon/redux-thunk}{(redux-thunk)}
...
redux-logger\href{https://github.com/evgenyrodionov/redux-logger}{(redux-logger)}
...
tcomb-form-native\href{https://github.com/gcanti/tcomb-form-native}{(tcomb-form-native)}
...
Mocha\href{https://mochajs.org/}{(Mocha)} was chosen as the testing
...
Chai\href{http://chaijs.com/}{(Chai)} is an assertion library for both
...
Istanbul\href{https://gotwarlost.github.io/istanbul/}{(Istanbul)} is a
...
GanttPro\href{https://ganttpro.com/}{(GanttPro)}, an online creator and
editor for Gantt charts, was used to create a timeline for the project,
while Google Docs\href{https://www.google.com/docs/about/}{(GoogleDocs)}
in conjunction with draw.io\href{https://www.draw.io/}{(draw.io)} were
used to capture requirements and create UML diagrams.\\
This report was originally written in markdown and transposed to the
Latex format using pandoc\href{http://pandoc.org/}{(pandoc)}.
...
Atom\href{https://atom.io/}{(Atom)} editor was chosen due to its level
...
Nuclide\href{https://nuclide.io/}{(Nuclide)} plugin, which enables Atom
...
Flowtype\href{https://www.flowtype.org/}{(Flowtype)}, a static type
...
ESLint\href{http://eslint.org/}{(ESLint)} was chosen as the linter for
...
For the smart contracts, written in Solidity, the
\texttt{solidity-linter}\href{https://atom.io/packages/linter-solidity}{(solidity-linter)}
Atom plugin was used, which avoided a significant amount of frustration
in comparison to waiting for errors to be detected at compile-time by
the Solidity compiler built into Eris's tooling.
...
DevTools\href{https://developers.google.com/web/tools/chrome-devtools/?hl=en}{(devTools)}
were utilised alongside the Xcode iOS
simulator\href{https://developer.apple.com/library/ios/documentation/IDEs/Conceptual/iOS_Simulator_Guide/Introduction/Introduction.html}{(iOS
Simulator)} in order to interact with the app and receive log outputs
...
The \texttt{git}\href{https://git-scm.com/}{(git)} command line utility
was used to manage the codebase's development over time, in conjunction
with GitHub\href{https://github.com/}{(GitHub)} to provide a remote
backup. For more involved \texttt{git} operations, such as merging
branches, Atlassian's
SourceTree\href{https://www.atlassian.com/software/sourcetree}{(SourceTree)}
...
Docker\href{https://www.docker.com/}{(Docker)} is a platform which
...
In a typical software engineering project, there is a need for strong
levels of communication between all the stakeholders involved in the
endeavour, in order to ensure all the needs the software should meet are
in fact met \emph{\href{}{(REF)}}. A frequent phenomenon within these
interactions is that requirements are revealed to the development team
in a haphazard way or are simply misinterpreted by the team
\emph{\href{}{(REF?)}}.\\
Fortunately these common pitfalls were a moot point within this project,
as the act of establishing requirements was entirely focused around the
...
MVP)\href{https://www.techopedia.com/definition/27809/minimum-viable-product-mvp}{(MVP;
better ref)} that an individual and/or group of individuals could use in
...
system\href{https://www.dsdm.org/content/moscow-prioritisation}{(MoSCoW)},
...
\textbf{Figure xx} below represents an overview of the uses cases that
...
deployment diagram \textbf{(fig X)}, which provided a high-level view of
...
\emph{``program to an interface, not an implementation''}\href{}{(REF)}.
Within the context of the web, the most appropriate form to follow this
approach was by developing a RESTful\href{}{(REF)} API, thus providing a
...
classes in object-oriented programming \emph{\href{}{(REF?)}}, with the
majority of the contracts either acting as
factories\href{http://www.oodesign.com/factory-pattern.html}{(factory
pattern; find better source)} for composite types or implementing
...
Principle\href{http://www.oodesign.com/single-responsibility-principle.html}{(SRP
pattern; find better source)} of object-oriented design, there was a
...
AngularJS\href{https://angularjs.org/}{(AngularJS)}. React's philosophy
...
principles\href{http://redux.js.org/docs/introduction/ThreePrinciples.html}{(three
principles)}:
...
\caption{State mutations within a Redux architecture}
\end{figure}

Source:
http://www.ibm.com/developerworks/library/wa-manage-state-with-redux-p1-david-geary/
...
Model-View-Controller\href{http://heaveneverywhere.com/stp/PostScript/mvc.pdf}{(Krasner,
Glenn E., and Stephen T. Pope. ``A description of the
model-view-controller user interface paradigm in the smalltalk-80
system.'' Journal of object oriented programming 1.3 (1988): 26-49.)}
(henceforth MVC) application with the important distinction that the
...
\caption{A typical MVC Model}
\end{figure}

Source: https://developer.chrome.com/apps/app\_frameworks
...
application may
require''\href{https://addyosmani.com/resources/essentialjsdesignpatterns/book/\#detailmvc}{(Learning
JS Design Patterns)}.\\
...
known as a hash map\href{}{(REF)}. This would have provided a sufficient
parallel to translate data formatted in JavaScript Object
Notation\href{http://www.json.org/}{(JSON)} (henceforth JSON) into a
...
an implementation of a SequenceArray\href{}{(\emph{Data Structures And
Algorithm Analysis in Java, p.~97})}, which became the
...
smell''\href{http://www.cs.wm.edu/~denys/pubs/ICSE'15-BadSmells-CRC.pdf}{(Tufano,
Michele, et al. ``When and why your code starts to smell bad.''
Proceedings of the 37th International Conference on Software
Engineering-Volume 1. IEEE Press, 2015.)}, by forcing the author to
...
database\href{https://msdn.microsoft.com/en-GB/library/ms179610.aspx}{(primaryKey/foreignKey)}.\\
...
place\href{https://addyosmani.com/resources/essentialjsdesignpatterns/book/\#detailmvc}{(Learning
JS Design Patterns)}. Within QuantiTeam, the server is able to fulfill
...
it\href{http://whatisrest.com/rest_constraints/index}{(REST)}. Within
...
server\href{http://whatisrest.com/rest_constraints/client_server}{(client-server)}.
...
request\href{http://whatisrest.com/rest_constraints/stateless}{(statelessness)}.\\
...
service\href{http://whatisrest.com/rest_constraints/cache}{(cache)}, as
...
architectures\href{http://whatisrest.com/rest_constraints/interface_uniform_contract}{(uniform
interface)}.\\
...
way\href{http://whatisrest.com/rest_constraints/layered_system}{(layered
system)}.\\
...
system's -- data
models\href{http://heaveneverywhere.com/stp/PostScript/mvc.pdf}{(Krasner,
Glenn E., and Stephen T. Pope. ``A description of the
model-view-controller user interface paradigm in the smalltalk-80
system.'' Journal of object oriented programming 1.3 (1988): 26-49.)}.
...
literal
types\href{https://flowtype.org/docs/builtins.html\#literal-types}{(Flowtype:literal
types)}. The \texttt{Tab} enum was useful to specify which string
...
actions\href{http://redux.js.org/docs/advanced/AsyncActions.html}{(Redux:AsyncActions)}:
...
manner\href{}{(Beck, Kent. Test-driven development: by example.
...
module\href{http://data.ceh.vn/Ebook/ebooks.shahed.biz/JAVA/JUNIT/Pragmatic\%20Unit\%20Testing\%20in\%20Java\%20with\%20JUnit\%20-\%20Andrew\%20Hunt\%20\&\%20David\%20Thomas\%20-\%20The\%20Pragmatic\%20Programmer.pdf}{(Hunt,
A., and D. Thomas. ``Pragmatic Unit Test: in Java with JUnit.'' (2003):
65-78.)}. This allowed the author to focus on ensuring that the API's
...
scheme\href{http://stackoverflow.com/help/whats-reputation}{(StackOverflow
reputation)}, QuantiTeam scores could become more meaningful the more
