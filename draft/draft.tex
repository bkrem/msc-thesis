...
disengagement and, over time, developed into a larger
...
verification and quantification of group-based work at large.\\
The major motivations that guided the project were thus twofold: in a broader sense, to
...
Working from this technological and philosophical motivation,
respectively, the following goals were established for the project:
\begin{itemize}
\item Create a public Application Programming Interface (henceforth API) to allow users to
interact with a blockchain in a well-defined manner, independent of a chosen client-side implementation.
\item Provide an iOS client-side app for basic task, user, and team management to offer a graphical
representation of the API in a format familiar to non-technical individuals.
\end{itemize}
...
for the project:
\begin{itemize}
\item Become familiar with native mobile app development by
utilising existing JavaScript knowledge in the context of React Native\cite{2react-native}.
\item Learn how to write smart contracts in Solidity\cite{1solidity} that are useful
in the context of the project and for existing platforms such as Ethereum\footnote{https://www.ethereum.org/}.
\item Implement a form of blockchain technology, by learning how to set up, run \& maintain a blockchain proficiently.
\item Gain a fundamental understanding of Docker\cite{1docker} and how it leverages ``containerisation'' to achieve straightforward deployments of development environments.
\end{itemize}

\section{1.4 Project Management}\label{project-management}

The project followed the methodology of the Unified Process\cite{jacobson1999unified} (henceforth
UP) framework for software
development, and was thus structured in terms of iterative
...
Issues\footnote{https://developer.github.com/v3/issues/} and organised into a Kanban\cite{1kanban} board with the help of waffle.io\footnote{https://waffle.io/}.
...
construct smart contracts for the blockchain), any responses coming from
...
\subsubsection{Chapter 1}\label{chapter-1-1}
...
far-off seeming concept of, for example, a high school diploma.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{../diagrams/educationalConditions.png}
...
\subsubsection{SocialX}\label{socialx}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../screenshots/socialx.png}
...
\subsubsection{WikiSpaces}\label{wikispaces}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../screenshots/wikispaces.png}
...
\subsubsection{HaikuLearning}\label{haikulearning}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../screenshots/haikulearning.jpg}
...
and students. It offers a unified environment to hand in assignments and
...
A blockchain is a data structure composed of a list of grouped transactions, known as blocks, where each block is linked back to the previous block in
...
repository of documentation for this specific type of blockchain and the computer science it is based on.
...
\subsubsection{eris-wrapper \& eris-logger}\label{eris-wrapper-eris-logger}
...
blockchain, while eris-logger simplifies setting up a logger with
...
React Native\cite{2react-native} for the project, which grafts a native mobile development
framework on top of the outstanding React library, along with the now
familiar Redux library.\\
React Native offers all the benefits of React while allowing the developer to hook
...
\subsubsection{redux-thunk}\label{redux-thunk}
...
\subsubsection{redux-logger}\label{redux-logger}
...
\subsubsection{tcomb-form-native}\label{tcomb-form-native}
...
blockchain was able to report errors within its own processes only in the most rudimentary of terms. This meant that if
...
\subsubsection{Mocha}\label{mocha}
...
\subsubsection{Static Type Analyser}\label{static-type-analyser}
...
JavaScript, where unintended type coercion is a common issue.
...
\subsubsection{Docker \& Shell Scripts}\label{docker-shell-scripts}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{../diagrams/conceptDiagram.png}
\caption{An early conceptual diagram demonstrating possible interactions
with the system}
\end{figure}

As outlined in chapters 1 \& 2, the key problem the project attempts
to solve is the way in which students relate to their school work on an
everyday basis by investigating the feasibility of a system
...
Figure 3.1 shows how the basic components and
...
\subsubsection{Factory Contracts}\label{factory-contracts}
...
\subsubsection{Data Structure Contract}\label{data-structure-contract}
...
\subsubsection{Manager Contracts}\label{manager-contracts}
...
\subsubsection{Linker Contract}\label{linker-contract}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{../diagrams/simplifiedContractsClassDiagram.png}
...
hexadecimal and UTF-8 encoding, for example. This is necessary due to Solidity's
...
\subsubsection{Uploader}\label{uploader}
...
\subsubsection{Abstracting view components with
...
UI components within a mobile app, providing ample opportunity to reuse said components even across different platforms. The decision to
...
\subsubsection{Managing state with Redux}\label{managing-state-with-redux}
...
state management, derived from the philosophy behind the Elm\footnote{http://elm-lang.org/} programming
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{../diagrams/mvc.png}
...
\subsubsection{Strings and Numbers}\label{strings-and-numbers}
...
\subsubsection{Arrays and Objects}\label{arrays-and-objects}
...
\subsubsection{Data: Factory Contracts}\label{data-factory-contracts}
...
\subsubsection{Relations: Linker Contract}\label{relations-linker-contract}
...
reliability and scalability for the web service implementing
...
\subsubsection{Client-Server Dichotomy}\label{client-server-dichotomy}
...
\subsubsection{Stateless}\label{stateless}
...
\subsubsection{Cacheable}\label{cacheable}
...
\subsubsection{Uniform Interface}\label{uniform-interface}
...
\subsubsection{Layered System}\label{layered-system}
...
state''\cite{osmani2012learning}, the parallels between the MVC pattern and
...
Flowtype elevated the ability to define actions and the
...
\subsubsection{Frequency \& Variability of
...
\subsubsection{Cross-Platform Potential}\label{cross-platform-potential}
...
\subsubsection{Synchronous UI Actions}\label{synchronous-ui-actions}
...
\subsubsection{Asynchronous I/O Actions}\label{asynchronous-io-actions}
...
constructed and expanded via test-driven development. This meant that
...
\subsubsection{Solidity Contracts \& Eris
...
\subsubsection{NodeJS \& Express}\label{nodejs-express}
...
\subsubsection{React Native, Redux \&
...
cryptographic hash may verify an entity or individual. The system is
...
would immediately feed back into a reward scheme. Similar to
